 ...
 */
package com.weisj.pj.view.abpullrefresh;
...
public class AbViewUtil {
    ...
    public static float dip2px(Context context, float dipValue) {
        DisplayMetrics mDisplayMetrics = SystemConfig.getDisplayMetrics(context);
	...
	public static int scaleValue(Context context, float value) {
		...
		if(mDisplayMetrics.scaledDensity > SystemConfig.UI_DENSITY){
			//密度
			if(width > SystemConfig.UI_WIDTH){
				value = value*(1.3f - 1.0f/mDisplayMetrics.scaledDensity);
			}else if(width < SystemConfig.UI_WIDTH){
